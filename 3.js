...
function task3() {
  ...
  Promise.allSettled([promise1(), promise2()])
    ...
    .then((results) => {
      ...
      //   console.log(`Проміс ${i + 1}: ${results[i].status}`);

      // можна згадати відсічення останнього слова )))
      // for (const result of results) {
      //   console.log(
      //     result.status === "fulfilled"
      //       ? result.value.replace(/\s+\S*$/, "") + ": " + result.status
      //       : result.reason.replace(/\s+\S*$/, "") + ": " + result.status
      //   );
      // }
    })
